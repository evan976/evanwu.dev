...
description: 'Learn how to build digital credentials using Solana Attestation Service, a new feature in Solana that allows you to create and verify digital credentials.'
publishedAt: '2025-11-01'
updatedAt: '2026-02-08'
image: '/articles/solana-attestation.webp'
topics: 'Solana, Attestation, Web3, TypeScript'
---
...
- [TypeScript](https://www.typescriptlang.org/) Experience
...
- **Revocable** - Attestations can be terminated by authorized signers defined in the Credential account.
...
- **User Verification** - ensure connected wallets are actual users by requiring signing a message (e.g., [Sign in with Solana](https://github.com/phantom/sign-in-with-solana) or [Supabase](https://supabase.com/))
...
- `getCreateCredentialInstruction`: will assemble the instruction to create the credential account ([ref](https://attest.solana.com/docs/helpers#create-credential-instruction)). For now, we will just add a single authorized signer, so that we can add our second signer later.
...
- `getCreateSchemaInstruction`: will assemble the instruction to create the schema account ([ref](https://attest.solana.com/docs/helpers#create-schema-instruction)).
...
- [Solana Program Derived Addresses (PDAs)](https://solana.com/docs/core/pda)
