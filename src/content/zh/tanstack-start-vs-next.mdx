...
topics: 'TanStack Start, Next.js, React, 路由'
---
...
**TanStack Start** 基于 **Vite Environment API** 构建，部署目标通过 Vite 插件配置。例如，你可以使用 Nitro 的 Vite 插件来部署到 Cloudflare Workers、Netlify 或 AWS Lambda：
...
关键区别：Nitro 不是 Start 的内置组件，而是通过 Vite 插件接入的部署适配器之一。
...
- Vite 驱动的开发体验，可插拔的部署适配器（Nitro、Cloudflare 等）。
...
- 路由命名规则严格——虽然路由器会监听文件变化并自动重新生成路由树，但命名规范有一定学习成本。
...
TanStack Start 和 Next.js 在解决类似问题，但哲学真的不同。Start 押注类型安全、URL-as-state 和 Vite 生态；Next.js 押注 Server Components、约定式架构和深度平台集成。
